...
def run_policy_on_tsplib_instance(
    instance_name: str,
    instance,
    run_seed: int,
    max_steps: int,
...
def summarize_results(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    summary["known_optimum"] = summary["instance"].map(TSPLIB_OPTIMA)

    summary["gap_to_optimum_percent"] = (
        (summary["mean_length"] - summary["known_optimum"])
        / summary["known_optimum"]
        * 100.0
    )

    summary["best_gap_to_optimum_percent"] = (
        (summary["best_length"] - summary["known_optimum"])
        / summary["known_optimum"]
        * 100.0
    )

    summary = summary.sort_values(["n_cities", "instance", "mean_length"])
...
def main():
    ...
    parser.add_argument(
        "--q-table-path",
        type=str,
        default="results/q_learning_v2/q_table_v2.pkl",
        help="Path to trained V2 Q-table.",
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Optional list of instance names, e.g. eil51 berlin52 kroA100.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of repeated runs per TSPLIB instance.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20_000,
        help="Offset for repeated run seeds.",
    )
    ...
    q_agent = load_q_learning_agent_v2(
        Path(args.q_table_path),
        seed=args.agent_seed,
    )

    tsp_files = list_tsplib_files(data_dir)

    if args.instances:
        wanted = {name if name.endswith(".tsp") else f"{name}.tsp" for name in args.instances}
        tsp_files = [path for path in tsp_files if path.name in wanted]

    if not tsp_files:
        raise FileNotFoundError(f"No TSPLIB .tsp files found in {data_dir}")
    ...
    for path in tsp_files:
        ...
        print(f"\nRunning TSPLIB instance: {instance_name} (n={instance.coords.shape[0]})")

        for run_idx in range(args.n_runs):
            run_seed = args.seed_offset + run_idx

            rows = [
                run_policy_on_tsplib_instance(
            ...
            print(
                ...
                f"length={best_row['tour_length']:.3f}"
            )

    raw = pd.DataFrame(all_rows)
    summary = summarize_results(raw)

    raw_path = out_dir / "tsplib_v2_raw.csv"
    summary_path = out_dir / "tsplib_v2_summary.csv"

    raw.to_csv(raw_path, index=False)
    summary.to_csv(summary_path, index=False)

    print("\nTSPLIB V2 comparison finished.")
